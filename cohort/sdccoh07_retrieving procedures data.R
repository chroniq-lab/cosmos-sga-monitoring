...
for(year in c(2010:2024)){
  print(year)
  print("Lookback")
  procedures_distinct_Year_index(con_Cosmos,project_string = "PROJECTS.ProjectD5A613.[ET4003\\shdw_1208_jvargh1].sdcdat201",IndexDateKey_Var = "earliest_datekey",
                                 LookBackInterval = 2,FollowUpInterval = 0,filter_year = year,
                                 by_codes = FALSE,cpt_codes=NULL,
                                 detection_string = "procd.CptCode") %>% 
    write_dataset(.,path=paste0(path_sga_dm_control_folder,"/working/sdccoh07a"),partitioning = c("Year"))
  
  print("Followup")
  procedures_distinct_YearMonth_index(con_Cosmos,project_string = "PROJECTS.ProjectD5A613.[ET4003\\shdw_1208_jvargh1].sdcdat201",IndexDateKey_Var = "earliest_datekey",
                                      LookBackInterval = 0,FollowUpInterval = 4,filter_year = year,
                                      by_codes = FALSE,cpt_codes=NULL,
                                      detection_string = "procd.CptCode") %>% 
    write_dataset(.,path=paste0(path_sga_dm_control_folder,"/working/sdccoh07b"),partitioning = c("Year"))
}
t - Sys.time()

open_dataset(paste0(path_sga_dm_control_folder,"/working/sdccoh07b"),partitioning = c("Year")) %>%
