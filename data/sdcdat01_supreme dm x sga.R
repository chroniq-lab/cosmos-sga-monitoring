...
FROM (
    ...
    WHERE PharmaceuticalClass LIKE '%ANTIPSYCHOTIC%'
	  AND PharmaceuticalSubClass LIKE '%Atypical%'
...
dbGetQuery(con_Cosmos, query) %>% 
  write_dataset(.,path=paste0(path_sga_dm_control_folder,"/working/sdcdat01"),partitioning = "Year")

open_dataset(paste0(path_sga_dm_control_folder,"/working/sdcdat01"),partitioning = "Year") %>% 
