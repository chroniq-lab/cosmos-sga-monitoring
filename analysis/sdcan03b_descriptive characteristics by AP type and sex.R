...
descriptive_df <- readRDS(paste0(path_sga_dm_control_folder,"/working/sdcan_imputed dataset with billing codes by AP type.RDS")) %>% 
  ...
  mutate(bmi_history_category = case_when(bmi_history >= 25 & bmi_history < 30 ~ "Overweight",
         ...
         insurance = case_when(insurance_medicare == 1 ~ "Medicare",
                               ...
                               TRUE ~ NA_character_),
         ap_type_sex = case_when(ap_type == "Neither" & female == 1 ~ "female Neither",
                                 ap_type == "SGA" & female == 1 ~ "female SGA",
                                 ap_type == "FGA" & female == 1 ~ "female FGA",
                                 ap_type == "SGA" & female == 0 ~ "male SGA",
                                 ap_type == "FGA" & female == 0 ~ "male FGA",
                                 TRUE ~ "male Neither")) %>% 
  mutate(ap_type_sex = factor(ap_type_sex,
                              levels=c("female Neither","male Neither",
                                       "female SGA","male SGA",
                                       "female FGA","male FGA"),
                              labels=c("female Neither","male Neither",
                                       "female SGA","male SGA",
                                       "female FGA","male FGA")),
  ...
  (descriptives <- descriptive_df %>% 
      tbl_summary(by=ap_type_sex,
                  include = c(raceeth,Dmagediag,age,region,
      ...
      add_overall()) %>%
    as_gt() %>%
    gt::gtsave(filename = "analysis/sdcan03b_descriptive characteristics by AP type and sex.html")
