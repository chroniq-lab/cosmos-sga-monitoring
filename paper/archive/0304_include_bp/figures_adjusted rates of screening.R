...
fig_df <- bind_rows(read_csv(paste0(path_sga_dm_control_folder,"/working/sdcan05/sdcan05a_modeled probability of abc.csv")),
                    read_csv(paste0(path_sga_dm_control_folder,"/working/sdcan05/sdcan05b_modeled probability of complications.csv"))) %>% 
...
anout5_df <- fig_df %>% dplyr::filter(outcome == "Any 5 outcomes")
...
ggarrange(a1c_plot,
  ...
  ggsave(.,filename=paste0(path_sga_dm_control_folder,"/figures/modeled rates of outcomes screening adjust for comorbidities by AP type.png"),width=12,height =5.5)
