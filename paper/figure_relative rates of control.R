...
fig_df <- read_csv(paste0(path_sga_dm_control_folder,"/working/sdcan07/sdcan07_model rates of control.csv")) %>% 
...
a1c_df <- fig_df %>% dplyr::filter(outcome == "HbA1c Control")
...
ldl_df <- fig_df %>% dplyr::filter(outcome == "LDL Control")
...
bp_df <- fig_df %>% dplyr::filter(outcome == "BP Control")
...
abc_df <- fig_df %>% dplyr::filter(outcome == "ABC Control")
...
ggarrange(a1c_plot,
  ...
  ggsave(.,filename=paste0(path_sga_dm_control_folder,"/figures/relative rates of control.jpg"),width=7,height =6.6)
...
ggarrange(a1c_plot,
          ...
          ncol=2,
          common.legend = TRUE,legend = "bottom") %>% 
  ggsave(.,filename=paste0(path_sga_dm_control_folder,"/figures/jpg/relative rates of control.jpg"),width=7,height =6.6)
